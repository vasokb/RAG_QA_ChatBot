...
def create_vectorDB(chunks, chunk_size, device, embedding_model = 'thenlper/gte-small', dist_strategy=DistanceStrategy.COSINE):
    ...
    embeddings = HuggingFaceEmbeddings(model_name=embedding_model, 
                                    ...
                                    encode_kwargs={"normalize_embeddings": True} # need to normalize the embeddings for cosine similarity
                                    )
    
    emb_model_max_seq_len = SentenceTransformer(embedding_model).max_seq_length
    if emb_model_max_seq_len < chunk_size:
        raise ValueError("The chunk_size is bigger than the max_seq_length of the embedding model! Choose a different embeddings model or change chunk_size.")  
    
    vectorDB = FAISS.from_documents(chunks, embeddings, distance_strategy=dist_strategy)
    return vectorDB    


st.cache_resource(ttl = '1d')
def load_llm(reader_model, device, dtype):
    """Load the LLM and tokenizer."""
    quantization_config = BitsAndBytesConfig(load_in_4bit=True) # load in 4bit, compute in float16
    ...
    READER_LLM = pipeline(
        ...
        do_sample=True, # Samples from the probability distribution of the predictions, instead of choosing token with the highest probability.
        temperature=0.1, 
...
def prompt_formatter(query, tokenizer, context_items):
    ...
    # TODO: Add examples to the template
